...
export class SpesialService {
    ...
    async newSpecial(body:SpesialDTO){
        try{
            if(!body.title){
                throw new HttpException("Название специальности не может быть пустым", HttpStatus.FORBIDDEN)
            }
    ...
    async deleteSpecial(id:string){
        try{
            ...
            if(isEvent){
                throw new HttpException("Специальность не может быть удалена, так как есть мероприятия связанные с ней", HttpStatus.FORBIDDEN)
            }
